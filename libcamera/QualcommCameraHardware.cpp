...
void cam_frame_flush_video (void)
{
    ...
    while (g_busy_frame_queue.front)
    {
       ...
       ALOGE("cam_frame_flush_video: node \n");
    }
    pthread_mutex_unlock(&(g_busy_frame_queue.mut));
    ALOGE("cam_frame_flush_video: out n = %d\n", g_busy_frame_queue.num_of_frames);
    return ;
}
...
static struct msm_frame * cam_frame_get_video()
{
    ...
    ALOGE("cam_frame_get_video... in\n");
    ALOGE("cam_frame_get_video... got lock\n");
    if (g_busy_frame_queue.front)
    {
       ...
       ALOGE("cam_frame_get_video... out = %x\n", p->buffer);
    }
    return p;
}
...
static void cam_frame_post_video (struct msm_frame *p)
{
    ...
    ALOGE("cam_frame_post_video... in = %x\n", (unsigned int)(p->buffer));
    pthread_mutex_lock(&(g_busy_frame_queue.mut));
    ALOGE("post_video got lock. q count before enQ %d", g_busy_frame_queue.num_of_frames);
    //enqueue to busy queue
    struct fifo_node *node = (struct fifo_node *)malloc (sizeof (struct fifo_node));
    if (node)
    {
        ALOGE(" post video , enqueing in busy queue");
        node->f = p;
        node->next = NULL;
        enqueue (&g_busy_frame_queue, node);
        ALOGE("post_video got lock. q count after enQ %d", g_busy_frame_queue.num_of_frames);
    }
    else
    {
    ...
    ALOGE("cam_frame_post_video... out = %x\n", p->buffer);
...
void QualcommCameraHardware::storeTargetType(void) {
    ...
    ALOGE(" Storing the current target type as %d ", mCurrentTarget );
    return;
}
...
bool QualcommCameraHardware::startCamera()
{
    ...
    ALOGE("loading liboemcamera at %p", libmmcamera);
    if (!libmmcamera) {
        ALOGE("FATAL ERROR: could not dlopen liboemcamera.so: %s", dlerror());
        return false;
    }
...
*/
    ...
    *(void **)&LINK_zoom_crop_upscale =
    ...
    if ((picture_sizes == NULL) || (!PICTURE_SIZE_COUNT)) {
     ...
     ALOGE("startCamera picture_sizes %p PICTURE_SIZE_COUNT %d", picture_sizes, PICTURE_SIZE_COUNT);
    ...
    ALOGE("startCamera preview_sizes %p previewSizeCount %d", preview_sizes, PREVIEW_SIZE_COUNT);
    ...
    ALOGE("startCamera hfr_sizes %p hfrSizeCount %d", hfr_sizes, HFR_SIZE_COUNT);
...
bool QualcommCameraHardware::initZslParameter(void)
    {  ALOGE("%s: E", __FUNCTION__);
       ...
       ALOGE("initZslParamter E: picture size=%dx%d", mPictureWidth, mPictureHeight);
       ...
       /* use the default thumbnail sizes */
        ...
        ALOGE("Number of snapshot to capture: %d",numCapture);
...
bool QualcommCameraHardware::initImageEncodeParameters(int size)
{
    ...
    bool ret;
    if (jpeg_quality >= 0) {
        ALOGE("initJpegParameters, current jpeg main img quality =%d",
             jpeg_quality);
    ...
    if (thumbnail_quality >= 0) {
        ...
        ALOGE("initJpegParameters, current jpeg thumbnail quality =%d",
             thumbnail_quality);
    ...
    if (rotation >= 0) {
        ALOGE("initJpegParameters, rotation = %d", rotation);
    ...
    if (mUseJpegDownScaling) {
      ALOGE("initImageEncodeParameters: update main image", __func__);
    ...
    mm_camera_parm_type_t type, uint16_t length, void *value, int *result)
{
    ...
    ALOGE("native_set_parms status = %d", status);
...
#define PARSE_LOCATION(what,type,fmt,desc) do {                                \
        ...
        ALOGE("GPS PARM %s --> [%s]", "gps-"#what, what##_str);                 \
        if (what##_str) {                                                      \
            type what = 0;                                                     \
...
#undef PARSE_LOCATION
    ...
    else ALOGE("not setting image location");
}
                         ...
                         bool register_buffer)
{
    ...
    ALOGE("register_buf:  reg = %d buffer = %p",
         !register_buffer, buf);
     ...
     //   ssize_t offset = offset_addr / mPreviewHeap->mAlignedBufferSize;
        ...
        int written;
                if (frameCnt >= 0 && frameCnt <= 10 ) {
                    ...
                    ALOGE("dumping preview frame %d", frameCnt);
                    if (file_fd < 0) {
        ...
        if( (mCurrentTarget != TARGET_MSM7630 ) &&  (mCurrentTarget != TARGET_QSD8250) && (mCurrentTarget != TARGET_MSM8660)) {
            if(rcb != NULL && (msgEnabled & CAMERA_MSG_VIDEO_FRAME)) {
                ...
                if (mReleasedRecordingFrame != true) {
                    ALOGE("block waiting for frame release");
                    mRecordWait.wait(mRecordFrameLock);
                    ALOGE("frame released, continuing");
                }
                mReleasedRecordingFrame = false;
            }
        }
        ...
        if ( mCurrentTarget == TARGET_MSM8660 ) {
            ...
            if (mFaceDetectOn == true && mSendMetaData == true) {
                ...
                if(mMetaDataHeap != NULL){
                    ALOGE("runPreviewThread mMetaDataHEap is non-NULL");
        ...
        if(bufferIndex >= 0) {
          ...
          ALOGE(" error Cancelling preview buffers  ");
...
void QualcommCameraHardware::runHFRThread(void *data)
{
    ...
    stopPreviewInternal();
    ALOGI("%s: setting parameters", __FUNCTION__);
    setParameters(mParameters);
    ALOGI("%s: starting Preview", __FUNCTION__);
...
void QualcommCameraHardware::runVideoThread(void *data)
{
        ...
        if(vframe != NULL) {
          ...
          frameCnt++;
...
#if 1
            ALOGE("in video_thread : got video frame, before if check giving frame to services/encoder");
            mCallbackLock.lock();
            int msgEnabled = mMsgEnabled;
            camera_data_timestamp_callback rcb = mDataCallbackTimestamp;
            void *rdata = mCallbackCookie;
            ...
            int index = mapvideoBuffer(vframe);
                ...
                if(rcb != NULL && (msgEnabled & CAMERA_MSG_VIDEO_FRAME) ) {
                    ALOGE("in video_thread : got video frame, giving frame to services/encoder index = %d", index);
                ...
                if(recordingState == 1) {
                    if(rcb != NULL && (msgEnabled & CAMERA_MSG_VIDEO_FRAME) ) {
                        ALOGE("in video_thread 3D mode : got video frame, giving frame to services/encoder");
                     ...
                     */
                    ALOGE("in video_thread 3D mode : got video frame, putting frame to Free Q");
    ...
    ALOGE("runVideoThread X");
}

void *video_thread(void *user)
{
    ALOGE("video_thread E");
...
LOGE("%s Got preview dimension as %d x %d ", __func__, previewWidth, previewHeight);
    ...
    ALOGE("initPreview E: preview size=%dx%d videosize = %d x %d", previewWidth, previewHeight, videoWidth, videoHeight );
    ...
    if( ( mCurrentTarget == TARGET_MSM7630 ) || (mCurrentTarget == TARGET_QSD8250) || (mCurrentTarget == TARGET_MSM8660)) {
        ...
        ALOGE("initPreview : preview size=%dx%d videosize = %d x %d", previewWidth, previewHeight,
    ...
    ALOGE("mDimension.prev_format = %d", mDimension.prev_format);
    ALOGE("mDimension.display_luma_width = %d", mDimension.display_luma_width);
    ALOGE("mDimension.display_luma_height = %d", mDimension.display_luma_height);
    ALOGE("mDimension.display_chroma_width = %d", mDimension.display_chroma_width);
    ALOGE("mDimension.display_chroma_height = %d", mDimension.display_chroma_height);
...
bool QualcommCameraHardware::initRawSnapshot()
{
    ...
    ALOGE("raw_snapshot_buffer_size = %d, raw_picture_height = %d, "\
         "raw_picture_width = %d",
...
bool QualcommCameraHardware::initZslBuffers(bool initJpegHeap){
    ...
    ALOGE("initZslBuffer: initializing mRawHeap.");
    if(mCurrentTarget == TARGET_MSM8660) {
    ...
    // Jpeg
    if (initJpegHeap) {
        ALOGE("initZslRaw: initializing mJpegHeap.");
        mJpegHeap =
    ...
    if( snapshotFormat == PICTURE_FORMAT_JPEG) {
        ...
        if (initJpegHeap)
        {
            for(int cnt = 0; cnt < numberOfJpegBuffers; cnt++)
            {
                ...
                if (mJpegfd[cnt] <= 0) {
                    ALOGE("%s: Open device for jpeg mem %s failed!\n",__func__, pmem_region);
                    return false;
                }
        ...
        for(int cnt = 0; cnt < (mZslEnable? (MAX_SNAPSHOT_BUFFERS-2) : numCapture); cnt++) {
            ...
            if((mPreviewWindow != NULL) && (mThumbnailBuffer[cnt] != NULL)) {
                ...
                ALOGE(" Locking thumbnail/postview buffer %d", cnt);
            ...
            if(mThumbnailBuffer[cnt]) {
                ...
                ALOGE("fd thumbnailhandle fd %d size %d", thumbnailHandle->fd, thumbnailHandle->size);
...
bool QualcommCameraHardware::initRaw(bool initJpegHeap)
{
    ...
    ALOGE("initRaw E: picture size=%dx%d", mPictureWidth, mPictureHeight);
    ...
    if(mCurrentTarget == TARGET_MSM7627 ) {
       ...
       ALOGE("the thumbnail sizes are %dx%d",mThumbnailWidth,mThumbnailHeight);
    }
    ...
    */
...
#if 0
    if (mJpegHeap != NULL) {
        ALOGE("initRaw: clearing old mJpegHeap.");
        mJpegHeap.clear();
    }
#endif
    //postview buffer initialization
    ...
    ALOGE("initRaw: initializing mRawHeap.");
...
void QualcommCameraHardware::deinitRawSnapshot()
{
    ...
    ALOGE("deinitRawSnapshot X");
}

void QualcommCameraHardware::deinitRaw()
{
    ALOGE("deinitRaw E");
    ALOGE("deinitRaw , clearing raw memory and jpeg memory");
    for (int cnt = 0; cnt < (mZslEnable? MAX_SNAPSHOT_BUFFERS : numCapture); cnt++) {
       if(NULL != mRawMapped[cnt]) {
         ALOGE("Unregister MAIN_IMG");
         register_buf(mJpegMaxSize,
...
void QualcommCameraHardware::relinquishBuffers()
{
    ...
    if( mPreviewWindow != NULL) {
      ...
      ALOGE(" PreviewWindow is null, will not cancelBuffers ");
    }
    mDisplayLock.unlock();
...
status_t QualcommCameraHardware::getBuffersAndStartPreview() {
    ...
    mFrameThreadWaitLock.lock();
    while (mFrameThreadRunning) {
        ALOGE("%s: waiting for old frame thread to complete.", __FUNCTION__);
        mFrameThreadWait.wait(mFrameThreadWaitLock);
        ALOGE("%s: old frame thread completed.",__FUNCTION__);
    }
    mFrameThreadWaitLock.unlock();

    if( mPreviewWindow!= NULL) {
        ALOGE("%s: Calling native_window_set_buffer", __FUNCTION__);
	...
	//yyan todo use handle to find out mPreviewBuffer
			...
			if(retVal == NO_ERROR) {
                ...
                if(handle) {
                  ...
                  ALOGE("fd mmap fd %d size %d", handle->fd, handle->size/*thumbnailHandle->size*/);
...
void QualcommCameraHardware::release()
{
    ...
    while (mSnapshotThreadRunning) {
        ALOGE("release: waiting for old snapshot thread to complete.");
        mSnapshotThreadWait.wait(mSnapshotThreadWaitLock);
        ALOGE("release: old snapshot thread completed.");
    }
    ...
    if(mCurrentTarget == TARGET_MSM8660) {
       ALOGE("release : Clearing the mThumbnailHeap and mDisplayHeap");
    ...
    mFrameThreadWaitLock.lock();
    while (mFrameThreadRunning) {
        ALOGE("release: waiting for old frame thread to complete.");
        mFrameThreadWait.wait(mFrameThreadWaitLock);
        ALOGE("release: old frame thread completed.");
    }
    mFrameThreadWaitLock.unlock();
...
status_t QualcommCameraHardware::startInitialPreview() {
   ...
   mFrameThreadWaitLock.lock();
    ...
    native_set_parms(CAMERA_PARM_DIMENSION,
              sizeof(cam_ctrl_dimension_t), &mDimension);
    ALOGE(" %s : mDimension.video_width = %d mDimension.video_height = %d", __FUNCTION__,
             mDimension.video_width, mDimension.video_height);
    mRecordFrameSize = previewFrameSize;
    ALOGE("mRecordFrameSize = %d", mRecordFrameSize);
...
{
   ALOGE("in startPreviewInternal : E");
...
#endif
    if(mCameraRunning) {
        ALOGE("startPreview X: preview already running.");
        return NO_ERROR;
    }
    if(mZslEnable){
         //call init
...
status_t QualcommCameraHardware::startPreview()
{
  ...
  if( mPreviewWindow == NULL) {
    ...
    ALOGE(" %s : Starting preview with initial buffers ", __FUNCTION__);
    result = startInitialPreview();
       ...
       * preview normally */
    ALOGE(" %s : Starting normal preview ", __FUNCTION__);
    result = getBuffersAndStartPreview();
  }
  ALOGE("startPreview X");
  return result;
}
...
void QualcommCameraHardware::stopInitialPreview() {
   mCameraRunning = 0;//!native_stop_ops(CAMERA_OPS_STREAMING_VIDEO, NULL);         
    ...
    ALOGE(" %s : E ", __FUNCTION__);
    if (mCameraRunning) {
        ALOGE(" %s : Camera was running. Stopping ", __FUNCTION__);
        {
            Mutex::Autolock l(&mCamframeTimeoutLock);
           {
      Mutex::Autolock cameraRunningLock(&mCameraRunningLock);
    ...
    if (!mCameraRunning) {
        ...
        if(mPreviewInitialized) {
            ...
            if( ( mCurrentTarget == TARGET_MSM7630 ) ||
                ...
                (mCurrentTarget == TARGET_MSM8660)) {
                mVideoThreadWaitLock.lock();
                ALOGE("in stopPreviewInternal: making mVideoThreadExit 1");
                mVideoThreadExit = 1;
                mVideoThreadWaitLock.unlock();
    ...
    if(err == NO_ERROR) {
        {
            ...
            if(mCameraRunning){
                ...
                ALOGE("As Camera preview is not running, AF not issued");
                status = false;
            }
        }
        mAfLock.unlock();
        ...
        ALOGE("As Cancel auto focus is in progress, auto focus request "
                "is ignored");
        status = FALSE;
    }
    {
...
{
    ALOGE("cancelAutoFocusInternal E");
    bool afRunning = true;

    if(!mHasAutoFocusSupport){
        ALOGE("cancelAutoFocusInternal X");
        return NO_ERROR;
    }
    ...
    do {
      ...
      if(err == NO_ERROR) {
          ...
          ALOGE("Auto Focus is not in progress, Cancel Auto Focus is ignored");
          mAfLock.unlock();
    ...
    ALOGE("cancelAutoFocusInternal X: %d", rc);
    return rc;
}

void *auto_focus_thread(void *user)
{
    ALOGE("auto_focus_thread E");
    CAMERA_HAL_UNUSED(user);
    QualcommCameraHardware *obj = QualcommCameraHardware::getInstance();
    if (obj != 0) {
        obj->runAutoFocus();
...
void QualcommCameraHardware::runSnapshotThread(void *data)
{
    ...
    if(strTexturesOn == true) {
        ...
        while (mJpegThreadRunning) {
            ALOGE("%s: waiting for jpeg callback.", __FUNCTION__);
            mJpegThreadWait.wait(mJpegThreadWaitLock);
            ALOGE("%s: jpeg callback received.", __FUNCTION__);
        }
        mJpegThreadWaitLock.unlock();
       ...
       if(!mZslEnable || mZslFlashEnable)
        ...
        ALOGE("runSnapshotThread : waiting for callback to come");
        mJpegThreadWaitLock.lock();
        while (mJpegThreadRunning) {
            ALOGE("%s: waiting for jpeg callback.", __FUNCTION__);
            mJpegThreadWait.wait(mJpegThreadWaitLock);
        ...
        mJpegThreadWaitLock.unlock();
        ALOGE("runSnapshotThread : calling deinitRawSnapshot");
        deinitRawSnapshot();
...
status_t QualcommCameraHardware::takePicture()
{
    ...
    mSnapshotThreadWaitLock.lock();
    while (mSnapshotThreadRunning) {
        ALOGE("takePicture: waiting for old snapshot thread to complete.");
        mSnapshotThreadWait.wait(mSnapshotThreadWaitLock);
        ALOGE("takePicture: old snapshot thread completed.");
    }
...
{
    ALOGE("initLiveSnapshot E");
//TODO
#if 0
    if (mJpegHeap != NULL) {
        ALOGE("initLiveSnapshot: clearing old mJpegHeap.");
        mJpegHeap.clear();
    }

    mJpegMaxSize = videowidth * videoheight * 1.5;

    ALOGE("initLiveSnapshot: initializing mJpegHeap.");
    mJpegHeap =
        new AshmemPool(mJpegMaxSize,
                       kJpegBufferCount,
                       0, // we do not know how big the picture will be
...
status_t QualcommCameraHardware::cancelPicture()
{
    ...
    if (mCurrentTarget == TARGET_MSM7627 ||
       (mCurrentTarget == TARGET_MSM7625A ||
        ...
        mSnapshotThreadWaitLock.lock();
        while (mSnapshotThreadRunning) {
            ALOGE("cancelPicture: waiting for snapshot thread to complete.");
            mSnapshotThreadWait.wait(mSnapshotThreadWaitLock);
            ALOGE("cancelPicture: snapshot thread completed.");
        }
        mSnapshotThreadWaitLock.unlock();
    }
...
{
    ALOGE("setParameters: E params = %p", &params);
...
#if 0
    if (mStatHeap != NULL) {
        ALOGE("setHistogram on: clearing old mStatHeap.");
        mStatHeap.clear();
    }
#endif
...
//                                   return NO_ERROR;
      ...
      case CAMERA_CMD_START_SMOOTH_ZOOM :
             ALOGE("HAL sendcmd start smooth zoom %d %d", arg1 , arg2);
             ...
             if(!mPreviewStopping) {
                 ...
                 ALOGE(" Not creating smooth zoom thread "
      ...
      case CAMERA_CMD_STOP_SMOOTH_ZOOM :
             ...
             mSmoothzoomThreadExit = true;
             mSmoothzoomThreadLock.unlock();
             ALOGE("HAL sendcmd stop smooth zoom");
             return NO_ERROR;
#endif 
   }
   return BAD_VALUE;
...
void QualcommCameraHardware::runSmoothzoomThread(void * data) {
    ...
    ALOGE("runSmoothzoomThread: Current zoom %d - "
    ...
    if(current_zoom == mTargetSmoothZoom) {
        ALOGE("Smoothzoom target zoom value is same as "
             "current zoom value, return...");
        if(!mPreviewStopping)
            mNotifyCallback(CAMERA_MSG_ZOOM,
                current_zoom, 1, mCallbackCookie);
        else
            ALOGE("Not issuing callback since preview is stopping");
        return;
    }
    ...
    while(1) {  // Thread loop
        ...
        if(mSmoothzoomThreadExit) {
            ALOGE("Exiting smoothzoom thread, as stop smoothzoom called");
            mSmoothzoomThreadLock.unlock();
            break;
        }
        mSmoothzoomThreadLock.unlock();
        ...
        if(!mPreviewStopping) {
            ...
            ALOGE("Preview is stopping. Breaking out of smooth zoom loop");
            break;
        }
    ...
    mSmoothzoomThreadRunning = false;
    mSmoothzoomThreadWaitLock.unlock();
    ALOGE("Exiting Smooth Zoom Thread");
}
...
QualcommCameraHardware* QualcommCameraHardware::createInstance()
{
    ...
    while (singleton_releasing) {
        if((singleton_releasing_start_time != 0) &&
            ...
            ALOGE("in createinstance system time is %lld %lld %lld ",
                    systemTime(), singleton_releasing_start_time, SINGLETON_RELEASING_WAIT_TIME);
            singleton_lock.unlock();
...
//#ifdef DUMP_LIVESHOT_JPEG_FILE
    ...
    else ALOGE("JPEG callback was cancelled--not delivering image.");
...
bool QualcommCameraHardware::initRecord()
{
    ...
    ALOGE("initREcord E");
    if(mZslEnable){
       ALOGE("initRecord X.. Not intializing Record buffers in ZSL mode");
       return true;
    }
    ...
    mVideoThreadWaitLock.lock();
    while (mVideoThreadRunning) {
        ALOGE("initRecord: waiting for old video thread to complete.");
        mVideoThreadWait.wait(mVideoThreadWaitLock);
        ALOGE("initRecord : old video thread completed.");
    }
    mVideoThreadWaitLock.unlock();
...
status_t QualcommCameraHardware::setDIS() {
    ...
    bool ret = true;
    ALOGE("mDisEnabled = %d", mDisEnabled);
    ...
    ALOGE("setDIS X (%d)", ret);
    return ret ? NO_ERROR : UNKNOWN_ERROR;
}

status_t QualcommCameraHardware::setVpeParameters()
{
    ALOGE("setVpeParameters E");

    video_rotation_param_ctrl_t rotCtrl;
    bool ret = true;
    ALOGE("videoWidth = %d, videoHeight = %d", videoWidth, videoHeight);
...
status_t QualcommCameraHardware::startRecording()
{
    ...
    if( (ret=startPreviewInternal())== NO_ERROR){
      ...
      if( ( mCurrentTarget == TARGET_MSM7630 ) || (mCurrentTarget == TARGET_QSD8250) || 
        ...
        ALOGE("frames in busy Q = %d", g_busy_frame_queue.num_of_frames);
        while((g_busy_frame_queue.num_of_frames) >0){
          msm_frame* vframe = cam_frame_get_video ();
          LINK_camframe_add_frame(CAM_VIDEO_FRAME,vframe);
        }
        ALOGE("frames in busy Q = %d after deQueing", g_busy_frame_queue.num_of_frames);
...
void QualcommCameraHardware::stopRecording()
{
    ...
    Mutex::Autolock l(&mLock);
    {
        ...
        if(mDataCallback && !(mCurrentTarget == TARGET_QSD8250) &&
            ...
            ALOGE("stopRecording: X, preview still in progress");
            return;
        }
    }
    ...
    if( ( mCurrentTarget == TARGET_MSM7630 ) || (mCurrentTarget == TARGET_QSD8250) || (mCurrentTarget == TARGET_MSM8660))  {
         ...
         */
        if(mIs3DModeOn) {
            ALOGE("%s: 3D mode on, so don't exit video thread", __FUNCTION__);
            recordingState = 0;
            return;
        }
        ...
        stopPreviewInternal();
    if (mJpegHeap != NULL) {
        ALOGE("stopRecording: clearing old mJpegHeap.");
        mJpegHeap.clear();
    }
#endif
    recordingState = 0; // recording not started
    ALOGE("stopRecording: X");
}
        ...
        for (cnt = 0; cnt < kRecordBufferCount; cnt++) {
            if(mStoreMetaDataInFrame){
                if(metadata_memory[cnt] && metadata_memory[cnt]->data == opaque){
                    ALOGE("in release recording frame(meta) found match , releasing buffer %d", (unsigned int)recordframes[cnt].buffer);
                    releaseframe = &recordframes[cnt];
                    break;
                }
            }else {
                if(recordframes[cnt].buffer && ((unsigned long)opaque == recordframes[cnt].buffer) ){
                    ALOGE("in release recording frame found match , releasing buffer %d", (unsigned int)recordframes[cnt].buffer);
                    releaseframe = &recordframes[cnt];
                    break;
                }
            }
...
static void receive_shutter_callback(common_crop_t *crop)
{
    ALOGE("receive_shutter_callback: E");
    QualcommCameraHardware* obj = QualcommCameraHardware::getInstance();
    if (obj != 0) {
        /* Just play shutter sound at this time */
        obj->notifyShutter(TRUE);
    }
    ALOGE("receive_shutter_callback: X");
}
                 ...
                 uint8_t *image, const char *name)
{
    ...
    if( bufDst > bufSrc ){
        ALOGE("crop yuv Y destination position follows source position");
        /*
    ...
    if( bufDst > bufSrc ) {
        ALOGE("crop yuv Chroma destination position follows source position");
        /*
         * Similar to y
         */
        int position = ( bufDst - bufSrc ) / (width - cropped_width);
...
void QualcommCameraHardware::receiveRawPicture(status_t status,struct msm_frame *postviewframe, struct msm_frame *mainframe)
{
    ...
    if(mSnapshotFormat == PICTURE_FORMAT_JPEG) {
        ...
        if(mThumbnailBuffer[index] != NULL) {
            ...
            ALOGE("%s: Queueing postview buffer for display %d",
                                           __FUNCTION__,handle->fd);
            if (GENLOCK_FAILURE == genlock_unlock_buffer(handle)) {
                ALOGE("%s: genlock_unlock_buffer failed", __FUNCTION__);
            }
        ...
        if (mDataCallback && (mMsgEnabled & CAMERA_MSG_RAW_IMAGE))
            ...
            ALOGE("ReceiveRawPicture : raw snapshot not Jpeg, sending callback up");
...
void QualcommCameraHardware::receiveJpegPicture(status_t status, mm_camera_buffer_t *encoded_buffer)
{
    ...
    if(encoded_buffer && status == NO_ERROR) {
      ...
      ALOGE("receiveJpegPicture: E buffer_size %d mJpegMaxSize = %d",buffer_size, mJpegMaxSize);
    ...
    if((index < 0) || (index >= (MAX_SNAPSHOT_BUFFERS-2))){
      ...
      ALOGE("receiveJpegPicture: Index of Jpeg is %d",index);
      ...
      if (mDataCallback && (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE)) {
            ...
            //  mDataCallback(CAMERA_MSG_COMPRESSED_IMAGE, buffer, mCallbackCookie);
              ...
              if(mCurrentTarget == TARGET_MSM8660){
                ...
                ALOGE("%s release memory",__FUNCTION__);
              } else{
    ...
    ALOGE("receiveJpegPicture: X callback done.");
}
...
status_t QualcommCameraHardware::setPreviewFrameRate(const CameraParameters& params)
{
    ...
    ALOGE("requested preview frame rate  is %u", fps);

    if(mInitialized && (fps == previousFps)){
        ALOGE("fps same as previous fps");
        return NO_ERROR;
    }
...
status_t QualcommCameraHardware::setPreviewFrameRateMode(const CameraParameters& params) {
    ...
    if(frameRateMode != NOT_FOUND) {
        ALOGE("setPreviewFrameRateMode: %s ", str);
...
bool QualcommCameraHardware::updatePictureDimension(const CameraParameters& params, int& width, int& height)
{
    ...
    ALOGE("updatePictureDimension: %dx%d <- %dx%d", width, height,
...
status_t QualcommCameraHardware::setSharpness(const CameraParameters& params)
{
    ...
    ALOGE("setting sharpness %d", sharpness);
...
status_t QualcommCameraHardware::setContrast(const CameraParameters& params)
{
    ...
    if(value == CAMERA_BESTSHOT_OFF) {
        ...
        ALOGE("setting contrast %d", contrast);
...
status_t QualcommCameraHardware::setSaturation(const CameraParameters& params)
{
    ...
    ALOGE("Setting saturation %d", saturation);
...
status_t QualcommCameraHardware::setSkinToneEnhancement(const CameraParameters& params) {
     ...
     if (mSkinToneEnhancement != skinToneValue) {
          ALOGE(" new skinTone correction value : %d ", skinToneValue);
...
{
#if 0
    ALOGE("constructing MemPool %s from gralloc memory: "
         "%d frames @ %d size "
         "buffer size %d",
         mName,
...
#endif
    ALOGE("Open MM camera DL libeomcamera loaded at %p ", libmmcamera);
    ALOGE("MMCameraDL: X");
}
...
QualcommCameraHardware::AshmemPool::AshmemPool(int buffer_size, int num_buffers,
    ...
    QualcommCameraHardware::MemPool(buffer_size,
                                    ...
                                    name)
{
    ALOGE("constructing MemPool %s backed by ashmem: "
         "%d frames @ %d uint8_ts, "
         "buffer size %d",
         mName,
         num_buffers, frame_size, buffer_size);
...
bool QualcommCameraHardware::register_record_buffers(bool register_buffer) {
    ...
    for (int cnt = 0; cnt < kRecordBufferCount; ++cnt) {
        ...
        ALOGE("register_buf:  reg = %d buffer = %p", !register_buffer,
          (void *)pmemBuf.vaddr);
        if(native_start_ops(register_buffer ? CAMERA_OPS_REGISTER_BUFFER :
                CAMERA_OPS_UNREGISTER_BUFFER ,(void *)&pmemBuf) < 0) {
            ALOGE("register_buf: MSM_CAM_IOCTL_(UN)REGISTER_PMEM  error %s",
        ...
        if( (strcmp("postview", mName) != 0) ){
            ...
            for (int cnt = 0; cnt < num_buf; ++cnt) {
                ...
                if(pmem_type == MSM_PMEM_VIDEO){
                     ...
                     ALOGE(" pmempool creating video buffers : active %d ", active);
                }
...
{
    ALOGE("destroying MemPool %s", mName);
    if (mFrameSize > 0)
        delete [] mBuffers;
    mHeap.clear();
    ALOGE("destroying MemPool %s completed", mName);
}
...
{
    ALOGE("receive_camframe_video_callback E");
    QualcommCameraHardware* obj = QualcommCameraHardware::getInstance();
    if (obj != 0) {
            obj->receiveRecordingFrame(frame);
         }
    ALOGE("receive_camframe_video_callback X");
}
...
bool QualcommCameraHardware::storePreviewFrameForPostview(void) {
    ALOGE("storePreviewFrameForPostview : E ");
     ...
     * for 7x30. */
    ALOGE("Copying the preview buffer to postview buffer %d  ",
         mPreviewFrameSize);
    if(mLastPreviewFrameHeap == NULL) {
    ...
    if( mLastPreviewFrameHeap != NULL && mLastQueuedFrame != NULL) {
        ...
        if(mUseOverlay && !mZslPanorama) {
            ...
            //if(mOverlay != NULL){
                ...
                ALOGE("Queueing Postview with last frame till the snapshot is done ");
...
#endif
    ALOGE("storePreviewFrameForPostview : X ");
    return true;
}
...
status_t QualcommCameraHardware::getBufferInfo(sp<IMemory>& Frame, size_t *alignedSize) {
    status_t ret;
    ALOGE(" getBufferInfo : E ");
    if( ( mCurrentTarget == TARGET_MSM7630 ) || (mCurrentTarget == TARGET_QSD8250) || (mCurrentTarget == TARGET_MSM8660) )
    {
    if( mRecordHeap != NULL){
        ALOGE(" Setting valid buffer information ");
        Frame = mRecordHeap->mBuffers[0];
        if( alignedSize != NULL) {
            *alignedSize = mRecordHeap->mAlignedBufferSize;
            ALOGE(" HAL : alignedSize = %d ", *alignedSize);
            ret = NO_ERROR;
        } else {
...
void QualcommCameraHardware::encodeData() {
    ...
    if (mDataCallback && (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE)) {
        mJpegThreadWaitLock.lock();
            ...
            mJpegThreadWaitLock.lock();
            while (mJpegThreadRunning) {
                ALOGE("encodeData: waiting for jpeg thread to complete.");
                mJpegThreadWait.wait(mJpegThreadWaitLock);
                ALOGE("encodeData: jpeg thread completed.");
            }
            mJpegThreadWaitLock.unlock();
    }
    else ALOGE("encodeData: JPEG callback is NULL, not encoding image.");
...
void QualcommCameraHardware::getCameraInfo()
{
    ...
    *(void **)&LINK_mm_camera_get_camera_info =
    ...
    if (libhandle) {
        ::dlclose(libhandle);
        ALOGE("getCameraInfo: dlclose(libqcamera)");
    }
#endif
